...
import { Search, Truck, User, CheckCircle, Clock, Camera, Eye, Package } from "lucide-react";
    ...
    <CardContent className="space-y-3">
      ...
      <div className="space-y-1 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span>{delivery.truck?.driver?.nombre || "N/A"}</span>
        </div>
      </div>
    </CardContent>
  </Card>
...
export function YardDeliveriesClientUI({ initialDeliveries }: { initialDeliveries: Delivery[] }) {
  ...
  useEffect(() => {
    if (selectedDelivery?.order?.items) {
      const initialItems = selectedDelivery.order.items.map((item: OrderItemType) => ({
  ...
  const handleLoadedQuantityChange = (pedido_item_id: string, quantity: number) => {
    setLoadedItems(prev => 
      prev.map(item => 
        item.pedido_item_id === pedido_item_id ? { ...item, dispatched_quantity: quantity } : item
      )
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardContent className="pt-6"><Input placeholder="Buscar por placa, ID o cliente..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></CardContent></Card>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4"><h3 className="font-semibold flex items-center gap-2"><CheckCircle className="h-5 w-5 text-green-500" />Cargados (Listos para Salir)<Badge variant="secondary">{loadedDeliveries.length}</Badge></h3>{loadedDeliveries.map(d => <DeliveryCard key={d.id} delivery={d} onSelect={() => {}} />)}</div>
      </div>

      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Confirmar Carga: {selectedDelivery?.truck?.placa}</DialogTitle><DialogDescription>Ingresa la cantidad real cargada para cada producto.</DialogDescription></DialogHeader>
          {selectedDelivery && (
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 bg-muted rounded-lg text-sm space-y-2">
                     ...
                     <div className="flex justify-between"><span>Pedido N°:</span><span className="font-medium">{selectedDelivery.order?.orderNumber}</span></div>
                  </div>
                  <div>
                  ...
                  {selectedDelivery.order?.items?.map((item: OrderItemType, index: number) => (
                    <div key={item.id} className={`p-3 grid grid-cols-3 gap-4 items-center ${index > 0 ? 'border-t' : ''}`}>
                      <div className="col-span-1">
                        ...
                        <p className="text-sm text-muted-foreground">Pedido: {item.cantidadSolicitadaBase} {item.product?.unit}</p>
                      </div>
                      <div className="col-span-2">
                        <Label htmlFor={`item-${item.id}`} className="text-xs mb-1">Cantidad Real Cargada *</Label>
                        <Input 
                          id={`item-${item.id}`}
                          type="number"
                          value={loadedItems.find(li => li.pedido_item_id === item.id)?.dispatched_quantity || ''}
