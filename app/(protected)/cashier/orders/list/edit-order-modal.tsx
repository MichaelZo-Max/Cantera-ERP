...
import type { Order, Client, Product, Destination, Truck, Driver } from "@/lib/types";
  ...
  useEffect(() => {
    if (isOpen && orderId) {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const [orderRes, clientsRes, productsRes, destinationsRes, trucksRes, driversRes] = await Promise.all([
          ...
          setOrderData(order);
          setCatalogs({ clients, products, destinations, trucks, drivers });
        } catch (error) {
          console.error(error);
