...
export async function POST(req: Request) {
  try {
    ...
    const { customer_id, destination_id, truck_id, items } = validation.data;

    const pedidoSql = `
  INSERT INTO CANTERA.rip.APP_PEDIDOS (customer_id, destination_id, truck_id, created_by, created_at, status)
  OUTPUT INSERTED.id
  VALUES (@customer_id, @destination_id, @truck_id, @created_by, GETDATE(), 'AWAITING_PAYMENT');
`;
    ...
    for (const item of items) {
      const itemSql = `
    INSERT INTO CANTERA.rip.APP_PEDIDOS_ITEMS (pedido_id, producto_id, quantity, unit_price)
    VALUES (@pedido_id, @producto_id, @quantity, @unit_price);
  `;
      const itemParams = [
